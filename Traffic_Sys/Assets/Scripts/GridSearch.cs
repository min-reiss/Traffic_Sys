...
public class GridSearch
{
    ...
    {
        Point applicant = list[0];
        foreach (Point vertex in list)
        {
            if (distanceMap[vertex] < distanceMap[applicant])
            {
                applicant = vertex;
            }
        }
    ...
    public static List<Point> GeneratePath(Dictionary<Point, Point> parentMap, Point endPoint)
    {
        List<Point> path = new List<Point>();
        Point parent = endPoint;
